...
def main():
	...
	for row in rowDataFromSensors:	
		...
		temp.append([row.id, row.date, row.date, 0, 1, row.x1, row.y1, row.z1, row.x2, row.y2, row.z2, row.id, row.temp, 1, 1, row.isOcc, row.mac])

		transformedData.append(TransformedData(row.mac, row.x1-test.x1, row.y1-test.y1, row.z1-test.z1, row.isOcc, row.date))


		#fid.wl(row.mac +","+ row.x1-test.x1 +","+ row.y1-test.y1, row.z1-test.z1, row.isOcc, row.date)
		# +","+ 
